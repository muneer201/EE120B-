...
unsigned char New_thresholds = 0;
unsigned char blue_blink = 0;
unsigned int blue_blink_counter = 0;
...
const unsigned long periodRightButton = 200;
const unsigned long periodBlueLED = 250;
...
int TickFct_RightButton(int);
int TickFct_BlueLED(int);
...
int TickFct_RightButton(int state) {
 ...
 switch (state) {
  ...
  case RBUTTON_WAIT:
   if (button) {
    state = RBUTTON_PRESSED;
   } else {
    state = RBUTTON_WAIT;
   }
   break;
  case RBUTTON_PRESSED:
   if (button) {
    state = RBUTTON_PRESSED;
   } else {
    state = RBUTTON_RELEASED;
   }
   break;
  case RBUTTON_RELEASED:
   state = RBUTTON_WAIT;
   break;
  default:
   state = RBUTTON_INIT;
 ...
 switch (state) {
  ...
  case RBUTTON_RELEASED:
   ...
   threshold_Min = (distance_CM * 4) / 5;
   threshold_Max = (distance_CM * 6) / 5;
   New_thresholds = 1;
   blue_blink = 1;
...
int TickFct_RedLED(int state) {
 ...
 switch (state) {
  ...
  case Red_LED:
   ...
   if (blue_blink) {
    PORTC &= ~(1 << 3);
   } else if (distance_CM < threshold_Min) {
    // 100%
    PORTC |= (1 << 3);
...
enum BlueLEDStates { BLUE_INIT, BLUE_OFF, BLUE_ON, BLUE_DONE };

int TickFct_BlueLED(int state) {
 switch (state) {
...
int main(void) {
 ...
 tasks[i].state = RBUTTON_INIT;
 tasks[i].period = periodRightButton;
 tasks[i].elapsedTime = 0;
 tasks[i].TickFct = &TickFct_RightButton;
 ++i;

 tasks[i].state = BLUE_INIT;
 tasks[i].period = periodBlueLED;
 tasks[i].elapsedTime = 0;
 tasks[i].TickFct = &TickFct_BlueLED;
